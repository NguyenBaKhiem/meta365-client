...
const ADDRESS = "0xbF7465Baa6b33A2aBf37a8C0F09B96a988C1632A";
const NFT_ADDRESS = "0x8aE4D4153bB1269471a994373008A01c761632bb";
const TOKEN_ADDRESS = "0x7237E7Fa09ecDB389a7893D02929fc320891DEDC";
...
const readNFT = new ethers.Contract(NFT_ADDRESS, NFT_ABI, getProvider());
...
const batchBuy = async (nft, ref) => {
  let ver = [];
  let nftAddress = [];
  for (let i = 0; i < nft.length; i++) {
    ver.push(721);
    nftAddress.push(NFT_ADDRESS);
  }
  const tx = await writeContract.batchBuy(ver, nftAddress, nft, ref);
  const hash = tx.hash;
  return getProvider()
    .waitForTransaction(hash)
    .then(() => {
...
const acceptOffer = async (nftId, buyerAddress) => {
  const tx = await writeContract.accept(
    721,
    NFT_ADDRESS,
    nftId,
    buyerAddress,
    "0x74E9dB8B88286b287fDfbFc7E706409D313a73e3"
  );
  const hash = tx.hash;
  return getProvider()
    .waitForTransaction(hash)
...
const batchSell = async (
  ...
  for (let i = 0; i < times; i++) {
    typesArr.push(721);
    addressArr.push(NFT_ADDRESS);
    unitArr.push(unit);
    priceArr.push(openPrice);
    closeArr.push(closePrice);
    startSaleArr.push(startTime);
    durationArr.push(duration);
    keepArr.push(keep);
  }
  const tx = await writeContract.batchSell(
    typesArr,
    addressArr,
    nftId,
    unitArr,
...
const batchMint = async (nftIds = [], owner = []) => {
  const gasLimit = await writeNFT.estimateGas.batchMintParent(nftIds, owner);
  const tx = await writeNFT.batchMintParent(nftIds, owner, {
    gasLimit: gasLimit,
  });
  const hash = tx.hash;
  return getProvider()
